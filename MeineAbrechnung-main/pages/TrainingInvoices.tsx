...
export const TrainingInvoices: React.FC<TrainingInvoicesProps> = ({ contacts }) => {
  ...
  const handleSaveToHistory = async (contact: Contact, method: 'email' | 'download', invNumber: string) => {
    try {
        ...
        console.error("Archiving invoice failed", e);
    }
  };
  ...
  const handleDownload = async () => {
      ...
      try {
          const nextInvStr = await db.getNextInvoiceNumber('RE');
          ...
          a.href = url;
          a.download = `Rechnung_${contactsWithInvoices.length === 1 ? contactsWithInvoices[0].contact.lastName : 'Sammel'}.pdf`;
      ...
      for (const id of selectedContacts) {
          ...
          try {
              ...
              const response = await fetch('/api/email/send', {
                    ...
                    body: JSON.stringify({
                        to: contact.email,
                        subject: `Rechnung: ${formData.title}`,
                        text: `Hallo ${contact.firstName},\n\nanbei die Rechnung für den Lehrgang ${formData.title}.\n\nViele Grüße\nDAV Alpinkader NRW`,
                        filename: `Rechnung_${contact.lastName}.pdf`,
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                    ...
                    <input type="number" step="0.01" value={formData.fee} onChange={e => setFormData({...formData, fee: parseFloat(e.target.value)})} className="w-full rounded-lg border-slate-300 border pl-10 pr-3 py-2" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Textvorlage</label>
                <textarea rows={10} value={formData.text} onChange={e => setFormData({...formData, text: e.target.value})} className="w-full rounded-lg border-slate-300 border px-3 py-2 font-mono text-sm" />
                <p className="text-xs text-slate-400 mt-1">Änderungen hier sind temporär. Globale Änderungen bitte unter Einstellungen.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
           <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 h-full flex flex-col">
              ...
              <div className="mt-6 pt-4 border-t border-slate-100 flex justify-between items-center">
                 <span className="text-sm text-slate-500">{selectedContacts.length} ausgewählt</span>
                 ...
                 <div className="flex gap-3">
                    ...
                    {selectedContacts.length > 0 && (
                         <button
                            ...
                            className="flex items-center gap-2 bg-slate-800 hover:bg-slate-900 text-white px-5 py-2.5 rounded-lg disabled:opacity-50 transition-colors decoration-0"
                         >
                            <Download className="w-4 h-4" /> {selectedContacts.length > 1 ? 'Sammel-PDF herunterladen' : 'PDF Vorschau'}
                         </button>
                    )}
